...
//	}
	 ...
	 */
	public boolean isInsctructorAvailable(Instructor instructor, List<BeginEndDTO> timeSlots)  {
		 for(BeginEndDTO dates : timeSlots) {
			TypedQuery<Session> query = em.createNamedQuery(Session.FIND_BY_INSTRUCTOR_AND_DATES, Session.class);
			query.setParameter(Session.INSTRUCTOR_OBJ, instructor);
			query.setParameter(Session.START_DATE, dates.getBegin());
			query.setParameter(Session.END_DATE, dates.getEnd());
